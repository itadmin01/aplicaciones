            ...
            for rec in i_nomina:
                ...
                if self.tipo_exp_idse == '0': #Alta / Reingreso
                   if rec.tipo_de_incidencia=='Reingreso' or rec.tipo_de_incidencia=='Alta':
                    ...
                    if len(employee.registro_patronal) != 11:
                       raise UserError(_('La longitud del registro patronal es incorrecto %s')  % (employee.name))
                    data1 = employee.registro_patronal[0:11].ljust(11, ' ') #Registro Patronal
                    ...
                    data5 = employee.apellido_Paterno and employee.apellido_Paterno.replace('ñ','#').replace('Ñ','#').ljust(27, ' ') or '                           ' #Primer apellido
                    data6 = employee.apellido_Materno and employee.apellido_Materno.replace('ñ','#').replace('Ñ','#').ljust(27, ' ') or '                           ' #Segundo apellido
                    if not employee.nombreEmpleado:
                       raise UserError(_('No tiene %s nombre de empleado configurado') % (employee.name))
                    data7 = employee.nombreEmpleado.replace('ñ','#').replace('Ñ','#').ljust(27, ' ') #Nombre(s)
                ...
                if self.tipo_exp_idse == '1': #Baja
                   if rec.tipo_de_incidencia=='Baja':
                    ...
                    data3 = employee.apellido_Paterno and employee.apellido_Paterno.replace('ñ','#').replace('Ñ','#').ljust(27, ' ') or '                           ' #Primer apellido
                    data4 = employee.apellido_Materno and employee.apellido_Materno.replace('ñ','#').replace('Ñ','#').ljust(27, ' ') or '                           ' #Segundo apellido
                    if not employee.nombreEmpleado:
                       raise UserError(_('No tiene %s nombre de empleado configurado') % (employee.name))
                    data5 = employee.nombreEmpleado.replace('ñ','#').replace('Ñ','#').ljust(27, ' ') #Nombre(s)
            ...
            if self.tipo_exp_sua == '0': ##Movmientos: ausentismo, baja, incapcidad, cambio sueldo
               ...
               for rec in i_nomina: # cambio de sueldo
                   if rec.tipo_de_incidencia=='Cambio salario':
                      ...
                      diasincidencia = '00'
                      if employee.contract_id:
                          data7='{:07d}'.format(int(round(employee.contract_id.sueldo_base_cotizacion,2)*100))
                      file_text.append((employee.registro_patronal[0:11] or '           ')+(employee.segurosocial[0:11] or '           ')+(data3)+(data4)+(folioimss)+(diasincidencia)+data7 + '\r')
            ...
            if self.tipo_exp_sua == '2':  ##Tipo alta
               for rec in i_nomina:
                   if rec.tipo_de_incidencia == 'Alta':
                      ...
                      nombre= nombre.replace('Ñ','¥')
                      if employee.contract_id:
                          sdi='{:07d}'.format(int(round(employee.contract_id.sueldo_base_cotizacion,2)*100))
